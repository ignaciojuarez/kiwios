...
Add the `kiwios-plugin` GitHub topic so the Plugins **Discover** community search can find the repository. Featured catalog inclusion is a separate reviewed-commit process and is not implied by the topic.

## Propose catalog inclusion

Submit one plugin or update per pull request by adding one entry to `catalog/catalog.json`; do not treat the schema's example as an approved entry. The plugin must live in its own public GitHub repository; do not catalog a path inside the KiwiOS application repository. Use the exact commit inspected above, a normalized repository URL of the form `https://github.com/owner/repository`, and the plugin path within that commit (`.` for the repository root). The entry's `id`, `version`, `kiwios_api`, and `license` must exactly match the manifest. Seed the catalog with the plugin repository's commit, not the KiwiOS commit that later contains `catalog.json`.
