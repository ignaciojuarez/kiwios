...
| `artifacts` | `checks.<id>` | same state as `table`; host adds per-row Install when the plugin depends on `native.artifact-delivery` |
...
- Plugins makes every lifecycle state visible. **Your plugins** lists installed sources. **Discover** below that lists featured catalog entries with a Reviewed label and Install, Installing, or Installed, then a community GitHub search that runs on submit. Community result cards carry a Community label and use the catalog name when the repository is featured. Featured install uses the catalog identity; community and the header **+** dialog send only a repository URL. The PWA renders a bounded, scrollable source/metadata/digest/disclosure review, including missing Homebrew formulae, and uses one identity-bound confirmation to install the source and queue those formulae. Catalog matches show that the entry was reviewed; that is not a warranty. A later retry of still-missing formulae uses Install packages, not a second Discover Install. Remote removal retains all Homebrew formulae and proceeds only when Keychain cleanup can remain interaction-disabled; package cleanup and any prompt-capable recovery remain in attended setup.
