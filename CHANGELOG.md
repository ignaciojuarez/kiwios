...
- Keep Monitor active when `macmon` reports idle Apple-silicon GPU averages below 10 °C. Sample a few readings, prefer a usable temperature, and warn with Idle instead of failing the plugin after three consecutive GPU-temperature errors. A repeated-execution Error no longer sticks after the plugin source itself changes.
- Add a non-bundled iOS build-library plugin that stays `configuration-required` until its public `library_root` is saved. Configure shows an orange warning that the folder is not sandboxed. Confirmed Clean old builds applies optional age and size retention. iPhone Safari can tap Install; KiwiOS revalidates the IPA and serves a short-lived OTA manifest from the existing Serve origin.
- Add a non-bundled Xcodes inventory/diagnostics reference plugin with compatibility-gated catalog parsing and fixture tests. Available-Xcode rows parse without an architecture suffix, catalog tables keep the newest 100 rows, and `xcodebuild -checkFirstLaunchStatus` runs before version/simctl probes. Install, selection, runtime-management, and authentication actions remain deferred behind attended-action and dynamic-choice contracts.
- Add explicit plugin setup requirements to the remote protocol and a shared Configure dialog. Cards no longer badge ordinary disabled state: configuration-required plugins show Configure in place of their switch, configured plugins show Configure below their description, and only authorization-required or error cards carry a status badge.

- Automatically collect prompt-free host and Homebrew inventory at launch, retain it for five minutes, and refresh stale remote snapshots without repeated polling scans. Tools and Brew now show a collecting state rather than asking for an initial manual refresh.

- Move page actions into the header: Home layout editing, host and Brew refresh, Doctor refresh, and Plugin add/reload. On phones the connection state sits beside the KiwiOS brand. Replace the Plugin sources card with a one-field GitHub repository dialog; KiwiOS resolves `HEAD` to the reviewed immutable commit at the repository root.

- Redesign web Plugin cards as a single two-column summary grid: name and manifest description on the left, lifecycle beside the name, and controls aligned right. Cards no longer show plugin status messages or logs. A not-added or dependency-incomplete plugin presents only Install; added plugins present Update when available, then the enable switch and Remove. Reject implausible Apple-silicon temperature values reported by `macmon` instead of displaying them as real readings.

- Show a clean version-aware Update control with an orange indicator for installed GitHub plugins whose default branch contains a newer manifest version. Replace separate Enable/Disable buttons with a Kiwi-colored switch and replace Remove text with an accessible trash icon.

- Add Monitor Home widgets for Apple-silicon CPU and GPU temperatures and the hottest readable SMART drive temperature. CPU and GPU temperatures use the rootless `macmon` Homebrew formula; the full drive-temperature table remains available.
- Let the web Plugins view review and enable bundled or development sources with the same identity-bound, one-use confirmation used by remote installs. It can also confirm and queue the approved plugin's exact missing Homebrew requirements locally on the Mac. Retry clears a repeated-execution failure after revalidation, and plugin cards no longer duplicate their failure message.

- Surface cancellation for running checks, actions, Watcher starts, and native jobs in the PWA. Permit a confirmation-bound, prompt-free remote restart of one revalidated current-user LaunchAgent; privileged restart remains attended-only.
...
- Make the web Plugins view a complete lifecycle control surface: reload, disable, retry, and staged immutable GitHub install/update run through reviewed, identity-bound one-use confirmations. Remote removal has its own review, retains all Homebrew packages, and deletes only interaction-disabled Keychain config accounts; prompt-capable recovery remains attended.
